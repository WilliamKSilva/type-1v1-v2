...
export class UserRepository implements IUserRepository {
    ...
    public async newUser({
        ...
        const createdUser = (await this.dbClient.query(`SELECT * FROM users WHERE uuid = $1`, [uuid])).rows.at(0);
    ...
    public async findUserByEmail(email: string): Promise<User | null> {
        const foundUser = (await this.dbClient.query(`SELECT * FROM users WHERE email = $1`, [email])).rows.at(0);
